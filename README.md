...
This module:
A. predicts visual styles in images generated by TTI models. We use a VGG16-based classifier fine-tuned on six style categories: **drawings**, **engravings**, **illustrations**, **paintings**, and **photography** (color or monochrome). Since WikiArt does not distinguish between color and monochrome photography, the script also computes a **colorfulness score** to separate them. 
B. calculates the **Visual Style Dominance (VSD)** score for each model and historical period based on the CNN predictions. The VSD score quantifies the extent to which a model converges on a single visual style when depicting a specific era.
