...
This module predicts visual styles in images generated by TTI models. We use a VGG16-based classifier fine-tuned on six style categories: **drawings**, **engravings**, **illustrations**, **paintings**, and **photography** (color or monochrome). Since the original dataset does not distinguish between color and monochroe photography, the script also computes a **colorfulness score** to separate them. Based on the predicted styles, the module then calculates the **Visual Style Dominance (VSD)** score for each model and historical period. The VSD score quantifies the extent to which a model converges on a single visual style when depicting a specific era.
